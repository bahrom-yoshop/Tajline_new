#!/usr/bin/env python3
"""
🎯 КРИТИЧЕСКОЕ ТЕСТИРОВАНИЕ: Исправления личного кабинета курьера в TAJLINE.TJ

ИСПРАВЛЕННЫЕ ПРОБЛЕМЫ:
1. Заявки принятые одним курьером больше не видны другим курьерам
2. Проверка логики получения новых заявок - исключение принятых и завершенных заявок

ДЕТАЛЬНОЕ ТЕСТИРОВАНИЕ:
1. Авторизация нескольких курьеров - создать тестовых курьеров
2. Создание тестовых заявок - создать заявки разных типов (delivery, pickup)
3. Проверка до принятия - все курьеры видят новые заявки
4. Принятие заявки курьером А - один курьер принимает заявку
5. Проверка после принятия - другие курьеры НЕ должны видеть принятую заявку
6. Проверка своих принятых заявок - курьер А должен видеть свои принятые заявки
7. Тестирование endpoint /api/courier/requests/new - разных курьеров
8. Тестирование endpoint /api/courier/requests/{request_id}/accept - принятие заявки

КОНТЕКСТ ИСПРАВЛЕНИЯ:
- Изменена логика в endpoint GET /api/courier/requests/new
- Добавлены фильтры исключающие принятые и завершенные заявки
- Заявки с assigned_courier_id=None и status=pending должны показываться всем
- Заявки с assigned_courier_id!=None не должны показываться другим курьерам

ОЖИДАЕМЫЕ РЕЗУЛЬТАТЫ:
- Курьер А принимает заявку → assigned_courier_id = A_id, status = accepted
- Курьер Б запрашивает новые заявки → не видит заявку принятую курьером А
- Курьер А запрашивает свои заявки → видит принятые им заявки
- Неназначенные заявки с status=pending видны всем курьерам

КРИТЕРИИ УСПЕХА:
✅ Заявка исчезает из списка новых заявок других курьеров после принятия
✅ Принявший курьер видит заявку в своих принятых
✅ Логика разграничения доступа работает корректно
"""
