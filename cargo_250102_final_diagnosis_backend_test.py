#!/usr/bin/env python3
"""
Final Backend Test for Cargo 250102 Visibility Diagnosis and Fix
...
class Cargo250102FinalDiagnosisTest:
    ...
    def analyze_cargo_destination(self):
        ...
        if 'душанбе' in recipient_address.lower() or 'dushanbe' in recipient_address.lower():
            ...
            # Would need Khujand warehouse ID
            print(f"⚠️ Определен город назначения: Худжанд (нужен warehouse_id)")
        else:
        ...
        if not cargo_found:
            try:
                ...
                if response.status_code == 200:
                    cargos = response.json()
                    for cargo in cargos:
                        if '250102' in cargo.get('cargo_number', ''):
                            final_cargo_data = cargo
                            cargo_found = True
                            print(f"✅ Груз найден через operator list")
                            break
            except Exception as e:
...
def main():
    """Main function"""
    test = Cargo250102FinalDiagnosisTest()
