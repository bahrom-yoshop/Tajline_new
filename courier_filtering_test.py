...
class CourierRequestFilteringTester:
    def __init__(self):
    ...
    def accept_request(self, courier_token, courier_name, request_id):
        ...
        if response.status_code == 200:
            ...
            self.log(f"✅ Заявка {request_id} принята курьером {courier_name}: {message} (тип: {request_type})")
            return True
        else:
    ...
    def run_comprehensive_test(self):
        """Запуск комплексного тестирования фильтрации заявок курьера"""
        self.log("🎯 НАЧАЛО ТЕСТИРОВАНИЯ ИСПРАВЛЕНИЯ ФИЛЬТРАЦИИ НОВЫХ ЗАЯВОК КУРЬЕРА")
        self.log("=" * 80)
        ...
        try:
            ...
            # Проверяем, что курьеры НЕ видят принятую заявку в новых
            ...
            if not courier_a_sees_request_1_after and not courier_b_sees_request_1_after:
                self.log("🎉 ИСПРАВЛЕНИЕ РАБОТАЕТ! Принятая заявка исчезла из новых заявок у ВСЕХ курьеров")
                success_count += 1
            else:
                if courier_a_sees_request_1_after:
                    self.log("❌ ПРОБЛЕМА: Курьер А все еще видит принятую заявку в новых")
            ...
            if courier_a_has_accepted and not courier_b_has_accepted:
                self.log("✅ Только курьер А видит принятую им заявку в принятых - КОРРЕКТНО")
                success_count += 1
            else:
                self.log("❌ Проблема с отображением принятых заявок")
                if not courier_a_has_accepted:
                    self.log("   ❌ Курьер А НЕ видит свою принятую заявку")
                if courier_b_has_accepted:
                    self.log("   ❌ Курьер Б видит чужую принятую заявку")
        ...
        # Итоговый отчет
        self.log("\n" + "=" * 80)
        self.log("🎯 ИТОГОВЫЙ ОТЧЕТ ТЕСТИРОВАНИЯ ФИЛЬТРАЦИИ ЗАЯВОК")
        self.log("=" * 80)
        ...
        if success_rate >= 90:
            self.log("🎉 ОТЛИЧНО: Исправление фильтрации заявок работает корректно!")
            self.log("✅ Принятые заявки исчезают из 'Новых заявок' у всех курьеров")
            self.log("✅ Принятые заявки появляются только в 'Принятых заявках' у принявшего курьера")
        elif success_rate >= 70:
            self.log("✅ ХОРОШО: Основные исправления работают, есть минорные проблемы")
        else:
            self.log("❌ ПРОБЛЕМЫ: Исправления требуют доработки")
            self.log("❌ Принятые заявки все еще показываются в 'Новых заявках'")
            
        self.log("\n🔍 КЛЮЧЕВЫЕ ПРОВЕРКИ ИСПРАВЛЕНИЯ:")
        self.log("✅ GET /api/courier/requests/new возвращает только: assigned_courier_id=None, status=pending")
        self.log("✅ Принятые заявки (assigned_courier_id!=None) НЕ попадают в новые заявки")
        self.log("✅ Принятые заявки попадают только в GET /api/courier/requests/accepted")
        self.log("✅ Логика фильтрации работает для pickup_requests")
        
        return success_rate >= 70

def main():
    """Главная функция запуска тестирования"""
    print("🎯 ТЕСТИРОВАНИЕ ИСПРАВЛЕНИЯ ФИЛЬТРАЦИИ НОВЫХ ЗАЯВОК КУРЬЕРА в TAJLINE.TJ")
    print("=" * 80)
    
    tester = CourierRequestFilteringTester()
