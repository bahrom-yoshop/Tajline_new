#!/usr/bin/env python3
"""
🎯 КРИТИЧЕСКОЕ ТЕСТИРОВАНИЕ: Полная функциональность генерации QR-кодов для транспорта в TAJLINE.TJ

ЗАДАЧА ИЗ REVIEW REQUEST:
Протестируй полную функциональность генерации QR-кодов для транспорта в TAJLINE.TJ:

1. **Авторизация администратора**: Войди под администратором
2. **Получение списка транспортов**: GET /api/transport/list 
3. **Создание заполненного транспорта для тестирования**: 
   - Найди транспорт со статусом "empty" 
   - Используй новый тестовый endpoint POST /api/transport/{transport_id}/set-filled чтобы сделать его заполненным
4. **Полная генерация QR-кода**: Протестируй POST /api/transport/{transport_id}/generate-qr для заполненного транспорта
   - Проверь что возвращается success: true
   - Убедись что qr_data содержит ТОЛЬКО ЦИФРЫ (это критично для пользователя)
   - Проверь что qr_code это валидный base64 image
   - Убедись что все поля присутствуют: success, transport_id, transport_number, qr_code, qr_data, message
5. **Проверка структуры числового QR кода**: 
   - qr_data должен быть в формате ТТТТПППССС (10 цифр)
   - ТТТТ - номер транспорта (4 цифры)
   - ППП - порядковый номер (3 цифры)
   - ССС - суффикс для уникальности (3 цифры)
6. **Повторная генерация**: Проверь что можно перегенерировать QR код для того же транспорта

КРИТИЧЕСКИ ВАЖНО:
Сфокусируйся на том, что QR-код содержит ТОЛЬКО ЦИФРЫ как требует пользователь - это критически важно!

ОЖИДАЕМЫЕ РЕЗУЛЬТАТЫ:
✅ Авторизация администратора работает
✅ GET /api/transport/list возвращает список транспортов
✅ Найден транспорт со статусом "empty"
✅ POST /api/transport/{transport_id}/set-filled успешно делает транспорт заполненным
✅ POST /api/transport/{transport_id}/generate-qr работает для заполненного транспорта
✅ qr_data содержит ТОЛЬКО ЦИФРЫ (10 цифр в формате ТТТТПППССС)
✅ qr_code это валидный base64 image
